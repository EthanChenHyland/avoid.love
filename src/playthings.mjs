...
export function playthings({state,wake,signal}){
 ...
 const current=state();air.draw(ctx,current);drawStorySpectacle(ctx,current);water.draw(ctx);drawKeepsakes(ctx,current);const {p,w,h,mobile,still,visitor:v}=current,next=p<.045?'opening':p>.967&&p<.985?'ending':'';
