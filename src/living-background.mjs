...
export function livingBackground({state,wake,signal}){
 ...
 let source='',wanted=false,pending=false,blocked=false,failed=false;
 ...
 video.addEventListener('loadeddata',wake,{signal});video.addEventListener('error',()=>{failed=true;pause();wake()},{signal});
 ...
 window.addEventListener('pagehide',()=>{pause();video.removeAttribute('src');video.load();source='';failed=false},{signal});
 return {get moving(){return wanted&&!failed&&!blocked&&!video.paused},get time(){return video.currentTime},get status(){return failed?'unavailable':blocked?'blocked':wanted?'playing':'paused'},sample(){
  ...
  if(!active)pause();
  if(!still&&p>=.97&&!source){source=`/films/morning-living-loop${mobile?'-mobile':''}.mp4`;video.src=source;video.load()}
  if(!active){if(p<.988&&video.readyState>=1&&video.currentTime)video.currentTime=0;return null}
  wanted=true;play();
  if(video.readyState<2||video.seeking||failed||blocked)return null;
