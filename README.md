...
The opening joins stationary film endpoints before starting the next camera move, avoiding the former long double-exposed zoom. Scroll-driven petals, memory-light ribbons, ink branches, orbiting letters and sunlight motes live in `src/story-spectacle.mjs`; they share the existing renderer, respond to visitor movement, and honor reduced motion.
