...
The effects are integrated into the scenes: petals arrive automatically, photographs fan out with scrolling and can be swiped in place, rain clears with scroll and touch, and the red thread follows pointer movement. There are no effect launchers or separate note/gallery dialogs. Reduced motion keeps the composition static. Run `QA_URL=http://127.0.0.1:4188/ node scripts/qa-playthings.mjs` to check phone, landscape and desktop behavior against the current preview.

Portrait layouts use continuous portrait artwork for the café and final passage to avoid changing composition between landscape movie frames and narrow crops. Coffee steam, rain trails, wind-responsive petals and photograph flips appear within the story. `qa-portrait-source.mjs` checks the embedded-panel-sized viewport and both sides of the mobile breakpoint.
