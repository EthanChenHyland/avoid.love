...
test('film progress is monotonic, clamped and reversible at every checkpoint',()=>{let prev=0;for(let i=0;i<=100;i++){const s=firstAct(i/100);assert.ok(s.film>=prev);assert.ok(s.film<=1);prev=s.film}assert.equal(firstAct(0).hero,1);assert.equal(firstAct(1).cafe,1);assert.equal(firstAct(.5).film,firstAct(.5).film);assert.equal(clamp(-1),0)});
