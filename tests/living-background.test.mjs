...
test('background warms lazily, plays only in the ending, pauses for overlays and resets away from it',async()=>{
 const original={document:globalThis.document,window:globalThis.window};let state={p:0,mobile:true,still:false,obscured:false},loads=0;
 const video={style:{},readyState:2,videoWidth:640,videoHeight:360,paused:true,currentTime:0,setAttribute(){},addEventListener(){},removeAttribute(){},load(){loads++},pause(){this.paused=true},play(){this.paused=false;return Promise.resolve()}};
 ...
 try{
  ...
  state.p=1;assert.equal(bg.sample(),video);await Promise.resolve();await Promise.resolve();assert.equal(video.muted,true);assert.equal(video.loop,true);assert.equal(bg.moving,true);
