...
### September 11 — final scroll pacing and scene processions

Added an isolated 180svh allocation at .997–1, preserving all earlier chapter allocations. The final heading's independent kinetic-word reveal now finishes at .998; avoid then disappears across .998–.99985. Replay text, accessible label and click behavior use the same .99985 threshold. Previously replay appeared at .985, before the finale.

Replaced the petal loop's explicit three-second gathered hold and one-second released hold with a continuous cosine cycle. Removed the second easing applied to its geometry, retained visible petals during release, and moved its activation earlier with a reversible scroll envelope. Native background continues its existing 18-second silent loop. Reduced motion still disables autonomous animation.

Added nine bounded scene processions: scarlet petals, curved ink strokes, cool rain-light traces and warm light trails. Large helical paths occupy mainly the lower scene plane and respond to pointer position. Their geometry depends on scroll position, so reverse scrolling retraces it without launching an animation. Mobile uses 48 rather than 84 elements. Original footage, chapter count, envelope and key pacing remain unchanged.

Validation: 31 unit tests pass, including reversible finite effect geometry, isolated scroll allocation and a finale phase without timed holds. Full layout sweep across eight viewport sizes reports no text collisions, clipping or overflow. 68 film/endpoint, letter interaction and reduced-motion checks pass. All 28 chapters remain available without JavaScript; offline-film fallbacks and reduced motion pass. Four-size ending checks verify scroll length, forward/reverse avoid opacity and replay timing. Continuous morning traversal reports no blanks/resets and maximum decoded frame step 2 across four sizes. Three-size native playback checks pass including loop wrap and reverse departure. Automated checks use Chromium, not physical iOS hardware.

Final build and revised heading/petal checks pass. The new fully readable avoid interval and replay threshold pass at 320, 390, 844 and 1280px widths; repeated finale playback and departure/reduced-motion suspension pass on mobile and desktop. Reviewed updated mobile finale and mobile/desktop earlier effects screenshots. A 30-boundary audit at four sizes sampled both directions: largest finite-interval canvas changes were 5.16% at .5763 and 4.62% at .16, with matching forward/reverse values. These are existing source-motion hotspots, not evidence of a direction-specific discontinuity; the audit does not claim all moving frames are pixel-identical or guarantee every source film is cut-free.
