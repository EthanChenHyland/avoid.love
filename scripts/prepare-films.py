...
for name,key in [('petal-transition','transition'),('poppy-film','opening')]:
 ...
 for tier,width in [('desktop',1280),('mobile',640)]:
  ...
  subprocess.run([ff,'-hide_banner','-loglevel','error','-i',str(src),'-vf',f'fps=15,scale={width}:-2','-c:v','libwebp','-quality','76','-start_number','0',str(dest/'%03d.webp')],check=True)
