...
for(const mode of ['offline-films','reduced','no-js']){
 ...
 if(mode==='no-js'){if(await p.locator('.scroll-track article').count()!==16)throw Error('No-JS chapters missing');out.push({mode,chapters:16});await p.close();continue}
 for(const at of [.185,.218,.27,.298,.37,.405,.487,.58,.628,.69,.736,.77,.855,.934,1]){await p.evaluate(at=>scrollTo(0,at*(document.querySelector('.scroll-track').offsetHeight-innerHeight)),at);await p.waitForTimeout(500);const d=await p.locator('#world').evaluate(c=>({...c.dataset}));if(mode==='reduced'&&d.film)throw Error('Reduced motion film');if(mode==='offline-films'&&+d.frame!==-1)throw Error('Offline film expected fallback');}
