...
for(const [width,height] of [[390,844],[1280,720]]){
 ...
 await go(1);await page.waitForTimeout(1700);await page.screenshot({path:`qa/finale/${width}-gather.png`});await page.waitForTimeout(3500);await page.screenshot({path:`qa/finale/${width}-bloom.png`});
 ...
 await page.waitForTimeout(8000);const loop=await draws();if(!loop)throw Error('Finale stopped after one cycle');
