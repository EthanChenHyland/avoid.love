...
try {for(const [width,height] of [[390,844],[1280,720]]){
 ...
 const result=await page.evaluate(async()=>{
  const world=document.querySelector('#world'),start=scrollY,extra=document.querySelector('.kept-scroll-room').offsetHeight,range=document.querySelector('.scroll-track').offsetHeight-innerHeight,span=.015*(range-extra-document.querySelector('.key-scroll-room').offsetHeight-document.querySelector('.stay-scroll-room').offsetHeight-document.querySelector('.clock-scroll-room').offsetHeight-document.querySelector('.address-scroll-room').offsetHeight-document.querySelector('.expansion-scroll-room').offsetHeight)+extra,samples=[];
