...
for(const width of [1280,390]){const p=await b.newPage({viewport:{width,height:width===390?844:720}});await p.goto((process.env.QA_URL||'http://127.0.0.1:4175/'));
for(const at of [.0667,.0805,.1955,.23,.255,.265,.31,.335,.42,.44,.54,.56,.5763,.6148,.65,.67,.75,.77,.82,.84,.8785,.885,.95,.966,.97]){
const pictures=[];for(const offset of [-.0003,.0003]){const target=at+offset;await scrollToChapter(p,target);await p.waitForFunction(at=>Math.abs(+document.querySelector('#world').dataset.progress-at)<.0002,target);await p.waitForTimeout(350);pictures.push(await p.locator('#world').screenshot())}
