...
for(const at of [.185,1]){
 await p.evaluate(at=>scrollTo(0,at*(document.querySelector('.scroll-track').offsetHeight-innerHeight)),at);await p.waitForFunction(at=>{const c=document.querySelector('#world');return Math.abs(+c.dataset.progress-at)<.0002&&+c.dataset.frame===120},at);await p.waitForTimeout(250);
