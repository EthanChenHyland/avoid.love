...
for(const [width,height] of [[390,844],[639,734],[701,900],[1280,720]]){
 ...
 for(const [film,at] of [['opening',.02],['transition',.12],['transition',.185],['transition',.23],['waiting',.37],['unsent',.487],['distance',.69],['kept',.77],['kept',.802],['impossible',.92],['impossible',.95],['stay',.976],['stay',1],['impossible',.95],['transition',.185]]){
 await go(at);await p.waitForFunction(film=>{const c=document.querySelector('#world');return c.dataset.film===film&&+c.dataset.frame>=0},film);await p.waitForTimeout(350);const data=await p.locator('#world').evaluate(e=>({...e.dataset}));if((at===1||at===.185)&&+data.frame!==(at===1?90:120))throw Error(`Endpoint not retained ${width} ${at}: ${data.frame}`);report.push({width,at,film,frame:data.frame,cache:data.cached,drawMs:data.drawMs});if([.185,.487,1].includes(at))await p.screenshot({path:`qa/film-continuity/${width}-${at}.png`})
 }
