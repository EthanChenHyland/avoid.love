...
for(const [width,height] of [[390,844],[844,390],[1280,720]]){
 ...
 await go(1);await p.waitForFunction(()=>document.querySelector('#living-background').currentTime>1&&document.querySelector('#world').dataset.backgroundReady==='true');const a=await p.locator('#living-background').evaluate(v=>({time:v.currentTime,videoWidth:v.videoWidth,muted:v.muted,loop:v.loop,duration:v.duration}));await p.waitForTimeout(1400);const time=await p.locator('#living-background').evaluate(v=>v.currentTime);if(time<=a.time)throw Error('Background did not animate while scroll was stationary');if(!a.loop||!a.muted)throw Error('Background must loop silently');
...
const p=await b.newPage({viewport:{width:390,height:844}});await p.route('**/films/morning-steady-loop*.mp4',r=>r.abort());await p.goto('http://127.0.0.1:4188/');await scrollToChapter(p,1);await p.waitForTimeout(1800);if(await p.locator('#world').getAttribute('data-film')!=='spare')throw Error('Missing background lost morning fallback');await p.close();await b.close();await fs.writeFile('qa/living-background/report.json',JSON.stringify(report,null,2));console.log(report);
