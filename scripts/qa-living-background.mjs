...
const p=await b.newPage({viewport:{width:390,height:844}});await p.route('**/films/morning-living-loop*.mp4',r=>r.abort());await p.goto('http://127.0.0.1:4188/');await scrollToChapter(p,1);await p.waitForTimeout(1800);if(await p.locator('#world').getAttribute('data-film')!=='spare')throw Error('Missing background lost morning fallback');await p.close();await b.close();await fs.writeFile('qa/living-background/report.json',JSON.stringify(report,null,2));console.log(report);
