...
for(const [width,height] of [[320,568],[390,844],[600,800],[700,900],[701,900],[768,1024],[844,390]]){
 ...
 await page.evaluate(p=>scrollTo({top:p*(document.querySelector('.scroll-track').offsetHeight-innerHeight),behavior:'instant'}),at);await page.waitForFunction(p=>Math.abs(+document.querySelector('#world').dataset.progress-p)<.0002,at);await page.waitForTimeout(450);
...
const page=await browser.newPage({viewport:{width:390,height:844}});await page.route('**/art/love-morning-mobile.webp',async route=>{await new Promise(r=>setTimeout(r,1600));await route.continue()});await page.goto(process.env.QA_URL||'http://127.0.0.1:4188/');await page.evaluate(()=>scrollTo(0,.9*(document.querySelector('.scroll-track').offsetHeight-innerHeight)));await page.waitForTimeout(600);await page.evaluate(()=>scrollTo(0,(document.querySelector('.scroll-track').offsetHeight-innerHeight)));await page.waitForTimeout(500);await page.screenshot({path:'qa/mobile-handoffs/late-ending.png'});await page.waitForTimeout(1600);await page.screenshot({path:'qa/mobile-handoffs/loaded-ending.png'});await page.close();await browser.close();await fs.writeFile('qa/mobile-handoffs/edges.json',JSON.stringify(report,null,2));console.log(`${report.length} forward/reverse samples passed; late ending captured`);
