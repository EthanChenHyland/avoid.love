...
import {chromium} from 'playwright';import fs from 'node:fs/promises';import path from 'node:path';import {narrative as chapters} from '../src/story.mjs';
...
const actual=await sw.evaluate(async ({factor,url})=>{const [t]=await chrome.tabs.query({url:new URL(url).origin+'/*'});await chrome.tabs.setZoom(t.id,factor);return chrome.tabs.getZoom(t.id)},{factor,url:process.env.QA_URL||'http://127.0.0.1:4175/'});
await p.waitForTimeout(250);const metrics=await p.evaluate(()=>({innerWidth,innerHeight,dpr:devicePixelRatio}));
for(const c of chapters){await scrollToChapter(p,c.at);await p.waitForFunction(at=>Math.abs(+document.querySelector('#world').dataset.progress-at)<.0002,c.at);await p.waitForTimeout(120);await fs.writeFile(`qa/zoom/${factor}-${c.id}.png`,Buffer.from((await cdp.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false,fromSurface:true})).data,'base64'));out.push({...await p.evaluate(inspectText),factor,actual,chapter:c.id,...metrics,...await p.locator('#world').evaluate(e=>({...e.dataset,overflow:document.documentElement.scrollWidth>innerWidth}))})}
}
